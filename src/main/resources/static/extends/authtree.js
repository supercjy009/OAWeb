    ...
    obj = {
        ...
        renderAuth: function (tree, dept, opt) {
            ...
            layui.each(tree, function (index, item) {
                ...
                var input;
                if (append) {
                    input = '<div style="padding-left: 8px;"  lay-skin="primary" lay-filter="' + layfilter + '" ' +
                        (item.checked ? ' checked="checked"' : '') + (item.disabled ? ' disabled' : '') + '>' + item.name + '</div>';
                } else {
                    input = '<input class="authtree-checkitem" type="' + opt.checkType + '" name="' + inputname + '" title="' + item.name + '" value="' + item.value + '" lay-skin="primary" lay-filter="' + layfilter + '" ' +
                        (item.checked ? ' checked="checked"' : '') + (item.disabled ? ' disabled' : '') + '>';
                }
